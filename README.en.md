...
The agent asks exactly one question at a time: ordered stops first, then a structured marker choice, then a landmark choice adapted to the available image-generation capability, and finally a concrete recommended or custom title. Recommended options appear first with short consequences and a free-form Other path. Recognized ferry legs switch to the bundled roll-on/roll-off ferry automatically.
...
The agent handles setup, map configuration, distance-aware cameras, adaptive regional or national overview framing, poster and pilot review, ferry logic, rendering, and QA.

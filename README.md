...
**城市地标**：系统先检测生图能力。可用时提供“智能推荐并生成 / 不展示 / 使用已有图片”；不可用时不显示自动生成选项，只提供“不展示 / 使用已有图片 / 先配置生图服务”。选择配置后会暂停地标分支，自动识别进程环境、项目 `.env` 和用户级私有配置；检测通过后再回到地标选择，不会同时偷跑无地标版本。生成前会确认完整地标清单和提示词。

...
- 首次到达放大地标，重复到达只点亮
- 开场先看完整路线，区域行程自动收紧视野，跨多区域长线才使用全国尺度；结尾慢速拉远并停留
...
**城市地标**：系统先检测生图能力。可用时提供“智能推荐并生成 / 不展示 / 使用已有图片”；不可用时不显示自动生成选项，只提供“不展示 / 使用已有图片 / 先配置生图服务”。选择配置后会暂停整个视觉渲染流程，自动识别进程环境、项目 `.env` 和用户级私有配置；检测通过后再回到地标选择，不会同时偷跑海报、样片、全片或无地标版本。生成前会确认完整地标清单和提示词。
